...
{if $md_download}
	{php} $this->middlePanel(__('Master Download').' - '.__('Categories')); {/php}
		{section=cats}
			<div class="tbl2 bold">
...
{if $md_subcat}
	{if $access}
		{php} $this->middlePanel(__('Category')); {/php}
			<h4>{$cat.name}</h4>
		...
		{if $file}
			{php} $this->middlePanel(__('Files')); {/php}
				{section=file}
		...
		{/if}
	{else}
		{php} $this->middlePanel(__('Master Download').' - '.__('Error')); {/php}
...
{if $md_file}
	{if $file}
		{php} $this->middlePanel(__('File')); {/php}
