...
class GallerySett {
	...
	{
		$this->_cache = $this->_system->cache('settings', NULL, 'gallery');
		if ($this->_cache === NULL)
		{
			$query = $this->_pdo->getData('SELECT * FROM [gallery_sett]');
			foreach($query as $data)
			{
				$this->_cache[$data['key']] = $data['value'];
			}

			$this->_system->cache('settings', $this->_cache, 'gallery');
		}
		return $this->_cache;
	}
	...
	{	
		$this->_system->clearCache('gallery', array('settings'));
	}
	 ...
	 * @uses    Database
	 */
	public function update(array $gallery_sett)
	{
		foreach ($gallery_sett as $key => $value)
		{
			if (isset($this->_cache[$key]))
			{
				...
				$count = $this->_pdo->exec('UPDATE [gallery_sett] SET `value` = :value WHERE `key` = :key', array(
